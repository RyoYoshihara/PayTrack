@extends('layouts.app')
@section('page-title', 'ルール登録')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-800">ルール登録</h2>
        <p class="text-sm text-gray-500 mt-1">収支の自動生成ルールを登録します</p>
    </div>

    <form method="POST" action="{{ route('rules.store') }}" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5" x-data="{ recurrence: '{{ old('recurrence', 'monthly') }}' }">
        @csrf

        <div>
            <label for="title" class="block text-sm font-medium text-gray-700 mb-1">タイトル</label>
            <input type="text" name="title" id="title" value="{{ old('title') }}" required
                   class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition" placeholder="例：給与、家賃、電気代">
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="month" name="end_month" id="end_month" value="{{ old('end_month') }}"
