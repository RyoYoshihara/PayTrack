@extends('layouts.app')
@section('page-title', '口座編集')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-800">口座編集</h2>
        <p class="text-sm text-gray-500 mt-1">口座情報を変更します</p>
    </div>

    <form method="POST" action="{{ route('bank-accounts.update', $bankAccount) }}" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
        @csrf @method('PUT')

        <div>
            <label for="name" class="block text-sm font-medium text-gray-700 mb-1">口座名</label>
            <input type="text" name="name" id="name" value="{{ old('name', $bankAccount->name) }}" required
        ...
        <div>
            ...
            <input type="number" name="balance" id="balance" value="{{ old('balance', $bankAccount->balance) }}" required
                   class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
            <p class="mt-1 text-xs text-gray-400">実際の口座残高に合わせて更新してください</p>
        </div>
